Experiments where conducted in the following order
...
\begin{tabular}{l r}
	\subfloat[String Subsequence Kernel for different length k. Like it's algorithmic cousin the NGK, it performs well for relative small k, but less well for larger k (again, see appendix for all data).]{
...
&
	\subfloat[Varying $ \lambda $ shows that SSK is not that sensitive to values of $ \lambda $. Generally we can see that unless you choose more extreme values, the SSK performs well. ]{
			\begin{tabular}{| c | c | c | c | c | } \hline
...
\begin{tabular}{l r}
	\subfloat[F1 scores for approximate kernel validation. k = 5 and $ \lambda = 0.5 $. Lodhi's results in parenthesis. This allows us to verify our implementation of the approximation kernel, but is not used for extensive results analysis.]{
...
\subfloat[Lodhi's top ten performing classes with our results for the full dataset with an approximating kernel. For the SSK $ \lambda $ was held constant at $ 0.5 $.]{
